...
namespace task1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int prime = 0;//создаем новую переменную, чтобы посчитаем количество делителей числа
            for (int i = 0; i < n; i++)
            {
            ...
            for (int i = 0; i < n; i++)
            {
                int k = 0;//создаем новую переменную, чтобы посчитать количетво делителей числа
                for (int j = 1; j <= arr[i]; j++)//пробегаемся от 1 до самого числа, чтобы найти делителей данного числа
                {
                    if (arr[i] % j == 0)
                    {
                    ...
                    if (k == 2)
                    {
                        Console.Write(arr[i] + " ");//если количество делителей равно 2, то число является простым и выводим это число
                    }
                }
            }
        }
